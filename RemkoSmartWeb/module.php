    ...
    class RemkoSmartWeb extends IPSModule {
		...
		private function SetEvent($eventName) {
			If(!$eid = @IPS_GetObjectIDByName ($eventName, $this->InstanceID)) {
				...
				$script = 'RSW_GetValues('.$this->InstanceID.');';
		...
		public function GetValues() {
			...
			if (isset($data)) {
				...
				$profileName = "RSW_HeatingMode";
				if(boolval($values[7])) IPS_SetVariableProfileAssociation($profileName, 4, "Kühlen", "", -1);	
			...
			if (isset($data)) {
				...
				$hideSolar = false;
				if(floatval($values[2]) == 300.0) $hideSolar = true;
					
				IPS_SetHidden($this->GetIDForIdent('IDS1'),$hideSolar);
				IPS_SetHidden($this->GetIDForIdent('IDS2'),$hideSolar);
				IPS_SetHidden($this->GetIDForIdent('IDS3'),$hideSolar);
				IPS_SetHidden($this->GetIDForIdent('IDS5'),$hideSolar);
				IPS_SetHidden($this->GetIDForIdent('IDS6'),$hideSolar);
